...
{answer}
Mosets Tree do not have support for paid listings at the moment.
{/answer}
...
{question}How do I change the default map marker in Mosets Tree?{/question}

{answer}
Mosets Tree allows you to show a map with a marker in each listings. When you create a new listing, this marker is placed on a default location (Kuala Lumpur, Malaysia by default).
...
5. Look for the following 2 options in this tab: `map_default_lat` and `map_default_lng`. This is the default latitude and longitude value used to load the default marker position on the map.
6. Enter new longitude and latitude values corresponding to the new default coordinate you want the map to load.
7. Click the `Save` icon on top.

{/answer}

{question}How do I change the size of the map in details view.{/question}

{answer}
The size of the map in a listing details view are defined in this template file:
...
Changing the value of the width and height will resize the map in listing details view.
{/answer}
...
3. Look for the parameter named "_Number of listings under top level cats._".
...
1. Login to your site's back-end.
2. Go to: **Mosets Tree > Templates > banyan**
3. Set the parameter named "**Show Categories' images**" to `Yes`. This will display your category images in Index page.
...
CAPTCHA is disabled by default in Mosets Tree. To enable it, go to Mosets Tree's Configuration and check under the "Captcha" tab. This tab contains 5 options that let you control which form (Review, Contact Owner, Report Listing, Report Review and Recommend) uses CAPTCHA. Once you have enabled CAPTCHA in Joomla from the section above and turned on the relevant options here, you will see CAPTCHA appearing in each of the Mosets Tree forms you've selected to use CAPTCHA.
...
The new '**Import Images**' tool introduced in Mosets Tree 3, helps you to work around this issue by reading URLs from a custom field and convert it to a listing image.
...
2. Look for the following code near line 2684:

		$page_title = Text::sprintf( 'COM_MTREE_PAGE_TITLE_VIEWLINK', $link->link_name, $link->city, $link->state, $link->country, $link->postcode );

3. Replace the line above with the following code:
		...
		// Text::sprintf. Edit the line below to control how and where to
		...
		// Build the title
		$page_title = call_user_func_array(array('Joomla\\CMS\\Language\\Text','sprintf'), $title_parameters);

	The existing `setTitle($page_title, null, $link_id);` line below it stays unchanged.
...
This instruction is based on Mosets Tree 4.0, but the same approach works on recent versions.
