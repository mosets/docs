# Upgrade
 ...
 - [Upgrading From 3.0 To 3.8]({{version}}/upgrade#from-30-to-38)
 - [Upgrading To {{version}}]({{version}}/upgrade#to-36)
...
If you are few version behind the latest version, upgrading to the latest version requires you to perform multi-steps upgrade. For example, if you're currently running 2.0 on Joomla 1.0 and wish to upgrade to run {{version}} on Joomla 3.5, these are the steps to upgrade:
...
6. Upgrade to Joomla 3.5
7. [Upgrading From 3.0 To 3.8]({{version}}/upgrade#from-30-to-38)
...
Joomla 3.4 | Mosets Tree 3.6, 3.7, 3.8
...
> - As part of the migration, it mentions that you need to migrate Mosets Tree templates located at this path: `/components/com_mtree/templates/*`. You can skip this path so that your new site will use Mosets Tree 3.8's default `kinabalu` template. This is so that you can enjoy some of the new features introduced in Mosets Tree 3.5 and later.  

## Upgrading From 3.0 To 3.8 {#from-30-to-38}
If you're running Mosets Tree 3.0, you first need to upgrade your Joomla site from Joomla 2.5 to Joomla 3.5.

Once you’ve done that, the upgrade from MT 3.0 to 3.8 is again very straightforward, by installing the latest version of Mosets Tree 3.8 package, `pkg_mtree-3.8.x.zip`.
...
## Upgrading To 3.8 {#to-38}
This section shows how you can upgrade Mosets Tree from earlier version of 3.8.x to the latest version.

### Requirement
This upgrade requires Joomla! 3.5 and Mosets Tree 3.8.x.
...
The steps to upgrade from Mosets Tree 3.x to the latest version of Mosets Tree 3.8 are the same as installing a new extension. This method of upgrading will overwrite all Mosets Tree files including its template files. If you have done any template modification, back-up those files before you begin the upgrade. Mosets Tree templates files are located at:

- `/components/com_mtree/templates/kinabalu/`

1. Download the latest package for Mosets Tree 3.8.x. This is the same package you use to install Mosets Tree 3.8 on a new Joomla site, but here, we are going to use this to upgrade your copy of Mosets Tree 3.6+ to 3.8.x.
2. Log in to your site's back-end and go to:
	1. Goto "**Extensions -> Extension Manager**"
	2. Click "**Browse...**" to select "pkg_mtree-3.8.x.zip" from your computer.
...
### Upgrade completed
If you've reached this point, congratulation. Your upgrade has completed. You're now running the latest version of Mosets Tree 3.8 on Joomla 3.5.
...
UPDATE  `#__mt_config` SET  `value` =  '3.8.1' WHERE  `#__mt_config`.`varname` =  'version';
